...
if __name__ == "__main__":
    ...
    if 'sampling_rate' not in args:
        ...
        args['sampling_rate'] = float(args['sampling_rate'])

    if 'analysis_half_width' not in args:
        args['analysis_half_width'] = 20
    else:
        args['analysis_half_width'] = int(args['analysis_half_width'])

    if 'root_save_dir' not in args:
        args['root_save_dir'] = None

    if 'no_gzip' not in args:
        args['no_gzip'] = False
    else:
        args['no_gzip'] = bool(args['no_gzip'])
    ...
    if args.get('show_bg_only', False):
   # if args['show_bg_only']:
    ...
    if args.get('do_spectral_analysis', False):
   # if args['do_spectral_analysis']:
    ...
    t6 = time.time()
    remover.save_out(use_gzip=not args['no_gzip'], save_in_root=args.get('save_out_in_root', False), save_merged=args.get('save_merged', False))
    t7 = time.time()
